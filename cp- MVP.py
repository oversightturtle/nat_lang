...
# CHANGING <<<<<<<<< THE TEXT
for token in doc: ## FOR EVERY WORD
  didwrite = False
  if any(x.isalpha() for x in token.text): ## DOES IT CONTAIN LETTERS? IF SO THEN
    for ins in parts_of_speech.instances:
      if ((token.pos_ == ins.name) and (ins.s_c) and (token.ent_type_ == "")): # is a pos type we should 
        try: # try to change if word in thesarus
          print (token)
          xxx = the[str(token)]
          print (xxx)
          con_EXPORT.write(xxx)
          con_EXPORT.write(token.whitespace_)
          didwrite = True
        except KeyError: #word should change but no thes key
